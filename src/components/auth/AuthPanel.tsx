...
export function AuthPanel() {
  ...
  return (
    <section className="terrain-panel pixel-frame pixel-border relative z-10 w-full max-w-md p-6">
      <div className="grid h-12 w-12 place-items-center border border-ember-line bg-surface-2 text-warm-white">
        <Glyph name="ascend" size="lg" />
      </div>
      <h1 className="mt-6 text-4xl font-normal lowercase tracking-[-0.02em]">enter ascend</h1>
      ...
      <form className="mt-7 space-y-4" onSubmit={(event) => event.preventDefault()}>
        ...
        {message && <p className="pixel-frame border border-ember-line bg-surface-3 p-3 text-sm text-ash-muted">{message}</p>}
