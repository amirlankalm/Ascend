  ...
  return (
    <article className="mx-auto grid max-w-7xl gap-6 px-5 pb-16 pt-8 lg:grid-cols-[1fr_380px]">
      <section className="terrain-panel pixel-frame pixel-border p-6 md:p-8">
        ...
        <h1 className="mt-4 max-w-4xl text-[clamp(3rem,7vw,6.4rem)] font-semibold leading-[0.95]">{quest.title}</h1>
        ...
        <div className="pixel-frame mt-8 border border-ember-line bg-black/24 p-5">
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <Glyph name="quest" size="md" className="text-sun-orange" />
            Mission brief
          </h2>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-3">
          {["Name concrete examples", "Explain user + AI advantage", "Add one original local insight"].map((item, index) => (
            <div key={item} className="pixel-frame border border-ember-line bg-black/24 p-4">
      ...
      <aside className="space-y-4">
        ...
        <div className="terrain-panel pixel-frame p-5">
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <Glyph name="check" size="md" className="text-sun-orange" />
            Pass criteria
          </h2>
          <ul className="mt-4 space-y-3 text-sm leading-6 text-ash-muted">
            {quest.evaluationRubric.map((item) => (
              <li key={item} className="border-l border-sun-orange/70 pl-3">
                {item}
              </li>
            ))}
          </ul>
...
function Stat({ icon, label, value }: { icon: "clock" | "bolt"; label: string; value: string }) {
  return (
    <div className="pixel-frame border border-ember-line bg-black/24 p-4">
      <Glyph name={icon} size="sm" className="text-sun-orange" />
